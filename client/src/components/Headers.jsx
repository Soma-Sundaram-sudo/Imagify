  ...
  return (
    ...
    viewport={{once:true}}>
        ...
        transition={{delay:0.2,duration:0.8}}>
            <p>Best website for text to image generator</p>
        ...
        <motion.button className='sm:text-lg text-white bg-black mt-8 w-auto px-12 py-2.5 flex items-center gap-2 rounded-full'
        whileHover={{scale:1.05}}
        whileTap={{scale:0.95}}
        initial={{opacity:0}}
        animate={{opacity:1}}
        transition={{default : {duration : 0.5},opacity : {delay:0.8,duration:1}}} onClick={onClickHandler}>
            Generate Image 
        ...
        transition={{delay:1,duration:1}}>
            {Array(6).fill("").map((item,index)=>(
                <motion.img className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-10' src={index % 2 === 0 ? assets.sample_img_1 : assets.sample_img_2} key={index} width={70} whileHover={{scale:1.05,duration:0.1}}/>
            ))}
        </motion.div>
