  ...
  return (
    ...
    onSubmit={onSubmitHandler} className='flex flex-col min-h-[90vh] justify-center items-center'>
      ...
      <div className='relative'>
        ...
        <span className={`absolute bg-blue-500 h-1  left-0 bottom-0 ${loading ? "w-full transition-all duration-[10s]":"w-0"} `}/>
      </div>
    ...
    </div>
    {
      ...
      <input onChange={e => setInput(e.target.value)} value={input} placeholder='Describe what you want to generate' type='text' className='flex-1 bg-transparent outline-none ml-8 max-sm:w-20 placeholder-color'/>
      <button className='bg-zinc-900 px-10 sm:px-16 py-3 rounded-full text-white '>Generate</button>
    </div>
    }
